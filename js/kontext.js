...
var iconsContainer = document.body.querySelector('.icons');
// var iconHome = document.body.querySelector( '#home');
...
for( var i = 0, len = k.getTotal(); i < len; i++ ) {
    ...
    //icon default
    var icon = document.body.querySelectorAll('.icons li');
    icon[i].className = i === 0 ? 'active': '';
    icon[i].setAttribute('index', i);
    icon[i].onclick = function( event ) { k.show( event.target.getAttribute( 'index' ) ) };
    icon[i].ontouchstart =  function( event ) { k.show( event.target.getAttribute( 'index' ) ) };

    // iconHome.onclick = function( event ) { k.show( event.target.getAttribute( 'index' ) ) };
    // iconHome.ontouchstart =  function( event ) { k.show( event.target.getAttribute( 'index' ) ) };
}
...
k.changed.add( function( layer, index ) {
    ...
    //icons updated
    var icons = document.body.querySelectorAll( '.icons > *');
    for( var i = 0, len = icons.length; i < len; i++ ) {
        icons[i].className = i === index ? 'active' : '';
    }
} );
